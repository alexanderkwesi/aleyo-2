...
// ── PUBLISHED WEBSITE VIEWER ────────────────────────────────────────
const PublishedWebsiteViewer = () => {
...
const WebsitePreview = ({ project }) => {
  ...
  const renderLink = (url, linkType, children, props = {}) => {
    ...
    const href = linkType === 'internal' ? `/${url.replace(/^\//, '')}` : url;
  ...
  // Render text elements with links
...
const DesignStudio = ({
  ...
  const saveWebsiteToDatabase = useCallback(async () => {
    ...
    try {
      ...
      // Generate complete HTML for the published site
  ...
  const generateFullHTML = useCallback(() => {
    const renderLinkHTML = (url, linkType, content) => {
      ...
      if (linkType === 'email') return `<a href="mailto:${url}">${content}</a>`;
      if (linkType === 'phone') return `<a href="tel:${url}">${content}</a>`;
      if (linkType === 'anchor') return `<a href="#${url.replace(/^#/, '')}">${content}</a>`;
      const target = linkType === 'external' ? ' target="_blank"' : '';
      const rel = linkType === 'external' ? ' rel="noopener noreferrer"' : '';
      return `<a href="${linkType === 'internal' ? '/' + url.replace(/^\//, '') : url}"${target}${rel}>${content}</a>`;
    };
    ...
    const renderComponentHTML = (component) => {
      ...
      switch (component.type) {
        ...
        case 'features':
          return `<div style="padding: 48px 20px; ${styles}">
            <h2 style="text-align: center; margin-bottom: 48px; color: ${globalStyles.headingColor};">${component.content.title}</h2>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 32px; max-width: 1200px; margin: 0 auto;">
              ${component.content.items
                ?.map(
                  (item) => `
                <div style="padding: 32px; text-align: center; background: ${alpha(globalStyles.primaryColor, 0.05)}; border-radius: ${globalStyles.borderRadius};">
                  ...
                  <h3 style="color: ${globalStyles.primaryColor}; margin-bottom: 8px;">${item.title}</h3>
                  <p style="color: ${alpha(globalStyles.textColor, 0.7)};">${item.description}</p>
                </div>
              `
                )
                .join('')}
        ...
        case 'gallery':
          return `<div style="padding: 48px 20px; ${styles}">
            <h2 style="text-align: center; margin-bottom: 48px; color: ${globalStyles.headingColor};">${component.content.title}</h2>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 24px; max-width: 1200px; margin: 0 auto;">
              ${component.content.items
                ?.map(
                  (item) => `
                <div style="padding: 16px; text-align: center; background: ${alpha(globalStyles.primaryColor, 0.05)}; border-radius: ${globalStyles.borderRadius};">
                  ${item.image ? `<img src="${item.image}" alt="${item.title}" style="width: 100%; height: 200px; object-fit: cover; border-radius: ${globalStyles.borderRadius}; margin-bottom: 16px;" />` : `<div style="width: 100%; height: 200px; background: ${alpha(globalStyles.primaryColor, 0.2)}; border-radius: ${globalStyles.borderRadius}; margin-bottom: 16px; display: flex; align-items: center; justify-content: center;">📷</div>`}
                  <h4>${item.title}</h4>
                  <p style="color: ${alpha(globalStyles.textColor, 0.7)};">${item.description}</p>
                </div>
              `
        ...
        case 'contact':
          return `<div style="padding: 48px 20px; ${styles}">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 32px; max-width: 1000px; margin: 0 auto;">
              <div style="padding: 32px; background: ${alpha(globalStyles.primaryColor, 0.05)}; border-radius: ${globalStyles.borderRadius};">
                <input type="text" placeholder="Name" style="width: 100%; padding: 12px; margin-bottom: 16px; background: transparent; border: 1px solid ${alpha(globalStyles.textColor, 0.2)}; border-radius: ${globalStyles.borderRadius}; color: ${globalStyles.textColor};" />
                <input type="email" placeholder="Email" style="width: 100%; padding: 12px; margin-bottom: 16px; background: transparent; border: 1px solid ${alpha(globalStyles.textColor, 0.2)}; border-radius: ${globalStyles.borderRadius}; color: ${globalStyles.textColor};" />
                <textarea placeholder="Message" rows="4" style="width: 100%; padding: 12px; margin-bottom: 16px; background: transparent; border: 1px solid ${alpha(globalStyles.textColor, 0.2)}; border-radius: ${globalStyles.borderRadius}; color: ${globalStyles.textColor};"></textarea>
                <button style="width: 100%; padding: 12px; background: ${globalStyles.primaryColor}; border: none; border-radius: ${globalStyles.borderRadius}; color: white; cursor: pointer;">Send Message</button>
              </div>
              <div style="padding: 32px; background: ${alpha(globalStyles.primaryColor, 0.05)}; border-radius: ${globalStyles.borderRadius};">
                <h4 style="color: ${globalStyles.headingColor}; margin-bottom: 16px;">Contact Information</h4>
                <p style="margin-bottom: 8px; color: ${alpha(globalStyles.textColor, 0.8)};">📍 ${component.content.address}</p>
                <p style="margin-bottom: 8px; color: ${alpha(globalStyles.textColor, 0.8)};">📧 ${component.content.email}</p>
                <p style="color: ${alpha(globalStyles.textColor, 0.8)};">📞 ${component.content.phone}</p>
              </div>
            </div>
          </div>`;
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 32px; max-width: 1000px; margin: 0 auto;">
              ${component.content.plans
                ?.map(
                  (plan) => `
                <div style="padding: 32px; text-align: center; background: ${alpha(globalStyles.primaryColor, 0.05)}; border-radius: ${globalStyles.borderRadius};">
                  <h3 style="color: ${globalStyles.primaryColor};">${plan.name}</h3>
                  <h2 style="font-size: 2.5rem; margin: 16px 0;">${plan.price}</h2>
                  <hr style="margin: 16px 0; border-color: ${alpha(globalStyles.textColor, 0.1)};" />
                  ${plan.features?.map((f) => `<p style="color: ${alpha(globalStyles.textColor, 0.7)}; margin: 8px 0;">✓ ${f}</p>`).join('')}
                  ${
                    plan.buttonText
                      ? renderLinkHTML(
                          plan.buttonLink || '#',
                          plan.buttonLinkType || 'internal',
                          `<button style="width: 100%; margin-top: 24px; padding: 12px; background: ${globalStyles.primaryColor}; border: none; border-radius: ${globalStyles.borderRadius}; color: white; cursor: pointer;">${plan.buttonText}</button>`
                        )
                      : ''
                  }
                </div>
...
<head>
  ...
  <title>${currentProject?.name || websiteName || 'My Website'}</title>
  <style>
...
<body>
  ...
  }, [globalStyles, components, textElements, imageElements, currentProject, websiteName]);
  ...
  const renderComponent = useCallback(
    (component) => {
      ...
      const renderLink = (url, linkType, children) => {
        ...
        if (linkType === 'email')
          return (
            <a href={`mailto:${url}`} style={{ color: 'inherit', textDecoration: 'none' }}>
              {children}
            </a>
          );
        if (linkType === 'phone')
          return (
            <a href={`tel:${url}`} style={{ color: 'inherit', textDecoration: 'none' }}>
              {children}
            </a>
          );
        if (linkType === 'anchor')
            ...
            >
              ...
              {editingText === element.id ? (
                ...
                renderLinkPreview(
                  ...
                  <Typography
                    component="span"
                    sx={{ ...element.styles, cursor: 'pointer' }}
                    onClick={(e) => {
                      e.preventDefault();
                      if (!dragDropMode) setEditingText(element.id);
                    }}
      ...
      >
        ...
        <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1.5, pt: 3 }}>
          ...
          <Box>
            ...
            <Typography variant="caption" sx={{ color: alpha('#FFFFFF', 0.45) }}>
              Your website will be saved and given a unique URL
            </Typography>
          </Box>
        </DialogTitle>
        <DialogContent sx={{ pt: 2 }}>
          <Stack spacing={3}>
            ...
            >
              <Typography variant="caption">
                Your website will be published and accessible at your chosen URL.
              </Typography>
            </Alert>
          </Stack>
        </DialogContent>
        ...
        <Box sx={{ p: 2, overflow: 'auto', height: 'calc(100% - 64px)' }}>
          {selectedComponent && (
            <>
                ...
                <>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
                    ...
                    <FormControl size="small" sx={{ minWidth: 120 }}>
                      <Select
                        value={selectedComponent.content?.buttonLinkType || 'internal'}
                        onChange={(e) =>
                          handleUpdateComponentContent(
                            selectedComponent.id,
                            'buttonLinkType',
              ...
              {selectedComponent.type === 'nav' && (
                <>
                  <Typography variant="subtitle2" sx={{ color: alpha('#FFFFFF', 0.7), mb: 1 }}>
                    Navigation Items ({selectedComponent.content?.items?.length || 0})
                  </Typography>
                  {selectedComponent.content?.items?.map((item, idx) => (
                    ...
                    >
                      ...
                      <Button
                        ...
                        color="error"
                        startIcon={<Delete />}
                        onClick={() => handleDeleteComponentItem(selectedComponent.id, 'nav', idx)}
                        sx={{ mt: 1 }}
                      >
                        Delete
                      </Button>
                  ...
                  <Button
                    ...
                    variant="outlined"
                    startIcon={<Add />}
                    onClick={() => handleAddComponentItem(selectedComponent.id, 'nav')}
                    sx={{ mt: 1, color: 'white', borderColor: alpha('#FFFFFF', 0.2) }}
                  >
                    Add Navigation Item
                  </Button>
                  <FormControl fullWidth size="small" sx={{ mt: 2 }}>
                    <InputLabel sx={{ color: alpha('#FFFFFF', 0.7) }}>Alignment</InputLabel>
                    <Select
                      value={selectedComponent.content?.alignment || 'center'}
                      onChange={(e) =>
                        handleUpdateComponentContent(
                          selectedComponent.id,
                          'alignment',
              ...
              {selectedComponent.type === 'logo' && (
                <>
                  ...
                  <TextField
                    ...
                    onChange={(e) =>
                      handleUpdateComponentContent(selectedComponent.id, 'link', e.target.value)
                    }
                    sx={{
                      mb: 2,
                      input: { color: 'white' },
                  ...
                  {selectedComponent.content?.plans?.map((plan, idx) => (
                    ...
                    >
                      ...
                      <Box sx={{ display: 'flex', gap: 1, mb: 1 }}>
                        ...
                        <FormControl size="small" sx={{ minWidth: 100 }}>
                          <Select
                            value={plan.buttonLinkType || 'internal'}
                            onChange={(e) =>
                              handleUpdateComponentContent(
                                selectedComponent.id,
                                'buttonLinkType',
                      ...
                      <Box sx={{ display: 'flex', gap: 1 }}>
                        ...
                        <FormControl size="small" sx={{ minWidth: 100 }}>
                          <Select
                            ...
                            onChange={(e) => {
                              const newLinks = [...selectedComponent.content.links];
                              newLinks[idx] = { ...newLinks[idx], linkType: e.target.value };
                              handleUpdateComponentContent(selectedComponent.id, 'links', newLinks);
                            }}
                            sx={{
                              color: 'white',
                  ...
                  {selectedComponent.content?.socialLinks?.map((social, idx) => (
                    ...
                    >
                      ...
                      <Box sx={{ display: 'flex', gap: 1 }}>
                        ...
                        <FormControl size="small" sx={{ minWidth: 100 }}>
                          <Select
                            value={social.linkType || 'external'}
                            onChange={(e) => {
                              const newSocial = [...selectedComponent.content.socialLinks];
                              newSocial[idx] = { ...newSocial[idx], linkType: e.target.value };
                              handleUpdateComponentContent(
                                selectedComponent.id,
                                'socialLinks',
                                newSocial
          ...
          {selectedTextElement && !selectedComponent && (
            <>
              ...
              {(selectedTextElement.tag === 'a' || selectedTextElement.isLink) && (
                <>
                  <TextField
                    fullWidth
                    label="Link URL"
                  ...
                  />
                  <FormControl fullWidth size="small" sx={{ mb: 2 }}>
                    <InputLabel sx={{ color: alpha('#FFFFFF', 0.7) }}>Link Type</InputLabel>
                    <Select
                      value={selectedTextElement.linkType || 'internal'}
                      onChange={(e) =>
                        handleUpdateTextElement(selectedTextElement.id, {
              ...
              {selectedTextElement.isNav && (
                <Typography variant="body2" sx={{ color: alpha('#FFFFFF', 0.5), mb: 2 }}>
                  Navigation items separated by | (pipe). Use || for link URL and link type:
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body2" sx={{ mb: 2, color: 'rgba(255,255,255,0.7)' }}>
            Your website has been published and is now live at your chosen URL.
          </Typography>
          <Paper
...
export { PublishedWebsiteViewer, WebsitePreview };
